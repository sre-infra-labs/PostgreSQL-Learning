...
> Output on Primary Cluster (podpg-cls1-pg1) -
```
ajaydwivedi@Ajays-MacBook-Pro Office % podman exec -it podpg-cls1-pg1 bash
root@podpg-cls1-pg1:/# patronictl -c /etc/patroni/patroni.yml show-config podpg-cls1 | grep -A5 standby_cluster
...
+ Cluster: podpg-cls1 (7649402051775700970) --+-----------+----+-----------+------------------+
...
root@podpg-cls1-pg1:/# 
root@podpg-cls1-pg1:/# 
root@podpg-cls1-pg1:/# psql -h localhost -U postgres -c "select slot_name, slot_type, temporary, active, xmin, wal_status, synced from pg_replication_slots;"
      slot_name       | slot_type | temporary | active | xmin | wal_status | synced 
----------------------+-----------+-----------+--------+------+------------+--------
 podpg_cls1_pg2       | physical  | f         | t      |  804 | reserved   | f
 standby_cluster_slot | physical  | f         | t      |  804 | reserved   | f
 podpg_cls1_pg3       | physical  | f         | t      |  804 | reserved   | f
(3 rows)

root@podpg-cls1-pg1:/# 
```

> Output on Standby Cluster (podpg-cls1-pg4) -
```
ajaydwivedi@Ajays-MacBook-Pro Office % podman exec -it podpg-cls1-pg4 bash
root@podpg-cls1-pg4:/# patronictl -c /etc/patroni/patroni.yml show-config podpg-cls1 | grep -A5 standby_cluster
standby_cluster:
  host: 172.18.0.10
  port: 5432
...
podman exec podpg-cls1-pg1 psql -h 172.18.0.11 -U postgres -c "
...
```bash
podman exec podpg-cls1-pg1 psql -h podpg-cls1-pg1 -U postgres << "EOF"
...
```bash
podman exec podpg-cls1-pg1 psql -h podpg-cls1-pg1 -U postgres -c "
...
**From the primary leader (podpg-cls1-pg1):**

```bash
podman exec podpg-cls1-pg1 psql -h podpg-cls1-pg2 -U postgres postgres -c "
SELECT
  application_name,
  state,
  sync_state,
...
WHERE application_name = 'podpg-cls1-pg4';
"
# ✅ flush_lag_bytes and replay_lag_bytes must be 0
```

**From the standby leader (podpg-cls1-pg4):**

```bash
podman exec podpg-cls1-pg4 psql -h 172.18.0.14 -U postgres postgres -c "
SELECT
  status,
  sender_host,
...
podman exec podpg-cls1-pg1 psql -h 172.18.0.11 -U postgres postgres -At -c "
  ...
  FROM pg_stat_replication WHERE application_name='podpg-cls1-pg4';" && \
echo "=== Standby side (pg4 local apply) ===" && \
podman exec podpg-cls1-pg4 psql -h 172.18.0.14 -U postgres postgres -At -c "
...
```bash
root@podpg-cls1-pg4:/# # *************** WHEN PRIMARY CLUSTER IS ONLINE ********************************
root@podpg-cls1-pg4:/# patronictl list
+ Cluster: podpg-cls1 (7649402051775700970) ----+-----------+----+-----------+
| Member         | Host        | Role           | State     | TL | Lag in MB |
+----------------+-------------+----------------+-----------+----+-----------+
...
root@podpg-cls1-pg4:/# # *************** WHEN PRIMARY CLUSTER IS OFFLINE ********************************
root@podpg-cls1-pg4:/# patronictl list
+ Cluster: podpg-cls1 (7649402051775700970) ----+---------------------+----+-----------+
| Member         | Host        | Role           | State               | TL | Lag in MB |
+----------------+-------------+----------------+---------------------+----+-----------+
| podpg-cls1-pg4 | 172.18.0.14 | Standby Leader | in archive recovery |  2 |           |
+----------------+-------------+----------------+---------------------+----+-----------+
root@podpg-cls1-pg4:/# 
```
...
```
root@podpg-cls1-pg4:/# 
root@podpg-cls1-pg4:/# systemctl restart patroni
root@podpg-cls1-pg4:/# 
root@podpg-cls1-pg4:/# patronictl list
+ Cluster: podpg-cls1 (7649402051775700970) ------+----+-----------+
| Member         | Host        | Role   | State   | TL | Lag in MB |
+----------------+-------------+--------+---------+----+-----------+
| podpg-cls1-pg4 | 172.18.0.14 | Leader | running |  4 |           |
+----------------+-------------+--------+---------+----+-----------+
root@podpg-cls1-pg4:/# 
```
...
| podpg-cls1-pg2 | 172.18.0.12 | Sync Standby | stopped |    |   unknown |                  |
| podpg-cls1-pg3 | 172.18.0.13 | Replica      | stopped |    |   unknown | nofailover: true |
+----------------+-------------+--------------+---------+----+-----------+------------------+
 Maintenance mode: on
root@podpg-cls1-pg1:/# 
root@podpg-cls1-pg1:/# patronictl -c /etc/patroni/patroni.yml \
  edit-config podpg-cls1 --force \
  --set "standby_cluster.host=podpg-cls1-pg4" \
  --set "standby_cluster.port=5432" \
  --set "standby_cluster.primary_slot_name=standby_cluster_slot"
...
| Member         | Host        | Role           | State               | TL | Lag in MB | Tags             |
+----------------+-------------+----------------+---------------------+----+-----------+------------------+
| podpg-cls1-pg1 | 172.18.0.11 | Replica        | in archive recovery |  4 |         0 |                  |
| podpg-cls1-pg2 | 172.18.0.12 | Standby Leader | in archive recovery |  2 |           |                  |
...
| Member         | Host        | Role           | State     | TL | Lag in MB | Tags             |
+----------------+-------------+----------------+-----------+----+-----------+------------------+
| podpg-cls1-pg1 | 172.18.0.11 | Replica        | streaming |  4 |         0 |                  |
| podpg-cls1-pg2 | 172.18.0.12 | Standby Leader | streaming |  4 |           |                  |
| podpg-cls1-pg3 | 172.18.0.13 | Replica        | streaming |  4 |         0 | nofailover: true |
+----------------+-------------+----------------+-----------+----+-----------+------------------+

root@podpg-cls1-pg1:/# 
root@podpg-cls1-pg1:/# patronictl failover 
...
| podpg-cls1-pg3 | 172.18.0.13 | Replica        | streaming |  4 |         0 | nofailover: true |
+----------------+-------------+----------------+-----------+----+-----------+------------------+
Candidate ['podpg-cls1-pg1'] []: podpg-cls1-pg1
Are you sure you want to failover cluster podpg-cls1, demoting current leader podpg-cls1-pg2? [y/N]: y
2026-06-09 15:08:24.81776 Successfully failed over to "podpg-cls1-pg1"
+ Cluster: podpg-cls1 (7649402051775700970) ----+---------------------+----+-----------+------------------+
| Member         | Host        | Role           | State               | TL | Lag in MB | Tags             |
+----------------+-------------+----------------+---------------------+----+-----------+------------------+
| podpg-cls1-pg1 | 172.18.0.11 | Standby Leader | in archive recovery |  2 |           |                  |
...
| Member         | Host        | Role           | State     | TL | Lag in MB | Tags             |
+----------------+-------------+----------------+-----------+----+-----------+------------------+
| podpg-cls1-pg1 | 172.18.0.11 | Standby Leader | streaming |  4 |           |                  |
...
# ✅ Expected:
# | Member           | Host        | Role           | State     | TL | Lag in MB |
# | podpg-cls1-pg1   | 172.18.0.11 | Standby Leader | streaming | NN |     0     |
# | podpg-cls1-pg2   | 172.18.0.12 | Replica        | streaming | NN |     0     |
# | podpg-cls1-pg3   | 172.18.0.13 | Replica        | streaming | NN |     0     |
```
...
# ✅ Expected:
# | Member           | Host        | Role   | State   | TL | Lag in MB |
# | podpg-cls1-pg4   | 172.18.0.14 | Leader | running | NN |     0     |
```
...
In this topology — primary cluster `podpg-cls1-pg1/pg2/pg3` (Region A) and cascading async standby `podpg-cls1-pg4` (Region B) — replication lag must be measured at **two hops**, because podpg-cls1-pg4 streams from the primary cluster's leader via `standby_cluster`, not as a member of the primary Patroni cluster. `patronictl list` on podpg-cls1-pg1 will NOT show podpg-cls1-pg4 (it belongs to its own cluster DCS).

All `podman exec` commands below run psql as `root` using the container's `/root/.pgpass` — no `PGPASSWORD` env var needed.

### 1. From the Primary leader (podpg-cls1-pg1) — `pg_stat_replication`

Authoritative view. podpg-cls1-pg4 appears as a streaming client of podpg-cls1-pg1.

```bash
podman exec podpg-cls1-pg1 psql -h 172.18.0.11 -p 5432 -U postgres postgres -c "
SELECT
  application_name,
  client_addr,
  state,
...
What to read:
- `flush_lag_bytes` / `replay_lag_bytes` — how far podpg-cls1-pg4 is behind in **bytes** of WAL.
...
### 2. From the standby cluster leader (podpg-cls1-pg4) — receiver + replay LSNs

Confirms what podpg-cls1-pg4 has actually received and replayed:

```bash
podman exec podpg-cls1-pg4 psql -h 172.18.0.14 -p 5432 -U postgres postgres -c "
SELECT
  status,
  sender_host,
  pg_last_wal_receive_lsn()                                                  AS received_lsn,
...
What to read:
- `apply_lag_bytes` — WAL **received but not yet replayed** on podpg-cls1-pg4 (local apply backlog).
- `replay_lag_seconds` — clock skew of last committed tx vs now; useful when podpg-cls1-pg1 is idle (no traffic → still 0 lag bytes but `replay_lag_seconds` grows).
- `status = streaming` confirms the link is up.

> Tip: when the primary is idle, `pg_stat_replication` lags can read `NULL`/0 even though podpg-cls1-pg4 hasn't received recent activity. Combine **(1)** and **(2)**.

### 3. From patronictl on the standby cluster

podpg-cls1-pg4 runs its own Patroni cluster, so query it locally:

```bash
podman exec podpg-cls1-pg4 patronictl -c /etc/patroni/patroni.yml list
# | Member           | Host        | Role            | State     | TL | Lag in MB |
# | podpg-cls1-pg4   | 172.18.0.14 | Standby Leader  | streaming | NN | <X> MB    |
```

The `Lag in MB` column = bytes behind the upstream primary (podpg-cls1-pg1). Quickest sanity check, granular to MB.

### 4. Patroni REST API (good for monitoring/scraping)

```bash
# Primary leader's view of replicas (won't include podpg-cls1-pg4 unless queried directly)
...
# podpg-cls1-pg4 health endpoint returns xlog info
...
```bash
echo "=== Primary side (podpg-cls1-pg1 → podpg-cls1-pg4) ==="
podman exec podpg-cls1-pg1 psql -h 172.18.0.11 -U postgres postgres -At -c "
...
echo "=== Standby side (podpg-cls1-pg4 local apply) ==="
podman exec podpg-cls1-pg4 psql -h 172.18.0.14 -U postgres postgres -At -c "
...
|---|---|---|---|
| `flush_lag_bytes` (podpg-cls1-pg1 → podpg-cls1-pg4) | < 16 MB | 16–100 MB | > 100 MB |
| `replay_lag` (time) | < 5 s | 5–60 s | > 60 s |
| `apply_lag_bytes` on podpg-cls1-pg4 | < 8 MB | 8–50 MB | > 50 MB |
