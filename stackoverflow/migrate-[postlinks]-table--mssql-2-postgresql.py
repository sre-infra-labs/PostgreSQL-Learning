...
sql_query = "SELECT Id, CreationDate, PostId, RelatedPostId, LinkTypeId FROM dbo.PostLinks ORDER BY Id;"
...
for i in range(0, len(rows), BATCH_SIZE):
    ...
    values = [
        (row.Id, row.CreationDate, row.PostId, row.RelatedPostId, row.LinkTypeId)
        for row in batch
    ]
...
pg_cursor.close()
pg_conn.close()
print("✅ Data migration for postlinks completed.")
