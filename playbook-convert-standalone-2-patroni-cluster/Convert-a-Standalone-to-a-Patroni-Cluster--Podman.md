# Convert a Standalone to a Patroni Cluster — Podman Containers
...
|------------------|--------------|------------|-----------|----------------|
| podpg-cls2-pg1   | 172.18.0.21  | 2231       | 5441      | 8021           |
| podpg-cls2-pg2   | 172.18.0.22  | 2232       | 5442      | 8022           |
| podpg-cls2-pg3   | 172.18.0.23  | 2233       | 5443      | 8023           |
| podpg-cls2-pg4   | 172.18.0.24  | 2234       | 5444      | 8024           |
...
# Part 1 — Create the Podman Container (podpg-cls2-pg1)
...
> host UIDs. Creating a dedicated, world-writable directory for cls2 avoids UID permission
> conflicts on the bind mount.

```bash
# Run on ryzen9 HOST (not inside container)
sudo mkdir -p /stale-storage/share-stalestorage/pgbackrest_backups_cls2
sudo chmod 777 /stale-storage/share-stalestorage/pgbackrest_backups_cls2
...
```bash
podman volume create pg-cls2-data-pg1
podman volume create pg-cls2-logs-pg1

# Verify
podman volume ls | grep pg-cls2
```
...
podman ps --filter name=podpg-cls2-pg1
```

## 5. Open a shell inside the container

```bash
podman exec -it podpg-cls2-pg1 bash
...
pgbackrest --stanza=podpg-cls2 stanza-create

# End-to-end config check
pgbackrest --stanza=podpg-cls2 check

# First full backup
pgbackrest --stanza=podpg-cls2 --type=full backup

# Confirm
pgbackrest --stanza=podpg-cls2 info
EOF
```
...
ETCD_INITIAL_CLUSTER="podpg-cls2-pg1=http://172.18.0.21:2380"
...
# Part 5 — Add Replicas (podpg-cls2-pg2 and podpg-cls2-pg3)

> Perform all steps for each replica. Commands marked **[host]** run on ryzen9; all others run **inside the container**.

## podpg-cls2-pg2 (172.18.0.22)

### [host] Create volumes and container

```bash
podman volume create pg-cls2-data-pg2
podman volume create pg-cls2-logs-pg2

podman run -d \
  --name podpg-cls2-pg2 \
  --hostname podpg-cls2-pg2 \
...
podman exec -it podpg-cls2-pg2 bash
```

### [inside podpg-cls2-pg2] /etc/hosts
...
### [inside podpg-cls2-pg2] Install packages
...
### [inside podpg-cls2-pg2] Configure etcd (joins pg1)
...
ETCD_INITIAL_CLUSTER="podpg-cls2-pg1=http://172.18.0.21:2380,podpg-cls2-pg2=http://172.18.0.22:2380"
ETCD_INITIAL_CLUSTER_TOKEN="podpg-cls2-etcd"
...
```bash
# Run this on podpg-cls2-pg1
etcdctl --endpoints=http://172.18.0.21:2379 \
  member add podpg-cls2-pg2 --peer-urls=http://172.18.0.22:2380
```

```bash
# Back on podpg-cls2-pg2
systemctl enable --now etcd
etcdctl --endpoints=http://172.18.0.21:2379,http://172.18.0.22:2379 member list
```

### [inside podpg-cls2-pg2] Configure pgbackrest
...
### [inside podpg-cls2-pg2] Configure Patroni and join the cluster
...
## podpg-cls2-pg3 (172.18.0.23)

Repeat the same steps as podpg-cls2-pg2 with these substitutions:

| Item | pg2 value | pg3 value |
|------|-----------|-----------|
| Container name | podpg-cls2-pg2 | podpg-cls2-pg3 |
| IP | 172.18.0.22 | 172.18.0.23 |
| Volume names | pg-cls2-data-pg2, pg-cls2-logs-pg2 | pg-cls2-data-pg3, pg-cls2-logs-pg3 |
| ETCD_NAME | podpg-cls2-pg2 | podpg-cls2-pg3 |
...
etcdctl --endpoints=http://172.18.0.21:2379,http://172.18.0.22:2379 \
  member add podpg-cls2-pg3 --peer-urls=http://172.18.0.23:2380
```

**etcd.conf ETCD_INITIAL_CLUSTER for pg3:**
```
ETCD_INITIAL_CLUSTER="podpg-cls2-pg1=http://172.18.0.21:2380,podpg-cls2-pg2=http://172.18.0.22:2380,podpg-cls2-pg3=http://172.18.0.23:2380"
ETCD_INITIAL_CLUSTER_STATE="existing"
```
...
# Expected output:
# + Cluster: podpg-cls2 (xxxxxxxxxxxxxxx) ----+----+-----------+
# | Member           | Host            | Role    | State   | TL | Lag in MB |
# +------------------+-----------------+---------+---------+----+-----------+
# | podpg-cls2-pg1   | 172.18.0.21:5432| Leader  | running |  1 |           |
# | podpg-cls2-pg2   | 172.18.0.22:5432| Replica | running |  1 |         0 |
# | podpg-cls2-pg3   | 172.18.0.23:5432| Replica | running |  1 |         0 |
# +------------------+-----------------+---------+---------+----+-----------+
...
# Planned switchover
/usr/local/bin/patronictl -c /etc/patroni/patroni.yml switchover podpg-cls2

# Reinitialize a replica
/usr/local/bin/patronictl -c /etc/patroni/patroni.yml reinit podpg-cls2 podpg-cls2-pg2

# Pause / resume automatic failover
/usr/local/bin/patronictl -c /etc/patroni/patroni.yml pause  podpg-cls2
/usr/local/bin/patronictl -c /etc/patroni/patroni.yml resume podpg-cls2

# Edit DCS-stored cluster config
/usr/local/bin/patronictl -c /etc/patroni/patroni.yml edit-config podpg-cls2

# pgbackrest — full backup (run on leader)
podman exec podpg-cls2-pg1 sudo -u postgres \
  pgbackrest --stanza=podpg-cls2 --type=full backup
